...
const char clFile_decoding[] = "question2_02";
...
int main(int argc, char *argv[]) {
	...
	status = clSetKernelArg(kernel, 3, localWorkSize*sizeof(cl_float), NULL);
	status = clSetKernelArg(kernel, 4, localWorkSize*sizeof(cl_float), NULL);
	const unsigned int blockSize = imgLength/messageSize;
	status = clSetKernelArg(kernel, 5, sizeof(cl_uint), &blockSize);
	...
	clEnqueueReadBuffer(cmdQueue, bufferOutput, CL_TRUE, 0, countWG, outputMsg, 0, NULL, NULL);
