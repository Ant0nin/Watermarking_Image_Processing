...
const char clFile_encoding[] = "question3_02";
...
int main(int argc, char *argv[]) {
	...
	cl_mem bufferImageOriginal = clCreateImage2D(context, 0, &format, imageWidth, imageHeight, 0, NULL, &status);
	size_t origin[3] = { 0,0,0 };
	size_t region[3] = { imageWidth, imageHeight, 1 };
	status = clEnqueueWriteImage(cmdQueue, bufferImageOriginal, CL_FALSE, origin, region, 0, 0, image, 0, NULL, NULL);
	...
	status = clSetKernelArg(kernel, 7, sizeof(cl_mem), &bufferEcartType);
	...
	kernel = clCreateKernel(program, kernel_maxEcartType, &status);
